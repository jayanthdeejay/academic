...
%The objective of the project is to implement the following algorithm on GPU . As stated earlier, it was %developed by Aditya Deshpande and P J Narayanan as presented in the paper [1].\\
%\noindent\rule{16.5cm}{2.0pt}\\
%\textbf {Algorithm: String sort on GPU}\\
%\noindent\rule{16.5cm}{1.0pt}
%1: Input: String Array G, Index Array I\\
%2: Output: Shuffled Index Array O.\\
%3: \emph{k} = optimal key length 	\hspace{2.2cm}// \emph{8 for our platform} \\
%4: \textbf{M} = load prefix(G,\emph{k}, 0) \\
%5: \emph{offset} = \emph{k} \hspace{2cm}// \emph{load next prefix starting at offset}\\
%6: \textbf{B} = [0,0,0,....,0] \hspace{3.6cm}// \emph{Only one segment}\\
%7: \emph{segBytes} = compute\_bytes(\textbf{Seg})\\
%8: \textbf{K} = pack\_keys(\textbf{M,Seg},\emph{segBytes})\\
%9: \textbf{repeat}\\
%10: \hspace{0.5cm}radix\_sort(key: \textbf{K}, value: \textbf{I})\\
%11: \hspace{0.5cm}\textbf{F}=mark\_singletons(\textbf{K,O}) \hspace{0.2cm}// \emph{F = Flag, O = %Output}\\
%12: \hspace{0.5cm}//\emph{above step also writes index of singletons to output}\\
%13: \hspace{0.5cm}\textbf{D}=prefix\_scan(\textbf{F}) \hspace{1.8cm}// \emph{D=Destination Array}\\
%14: \hspace{0.5cm}\textbf{K,I} = scatter(\textbf{K, I}, flag: \textbf{F}, dest: \textbf{D}) \hspace{0.2cm}// %\emph{compaction}\\
%15: \hspace{0.5cm}\textbf{Seg}=generate\_segments(\textbf{K}) \\
%16: \hspace{0.5cm}\emph{segBytes}=compute\_bytes(\textbf{Seg})\\
%17: \hspace{0.5cm}\textbf{M}=load\_prefix(\textbf{G}, \emph{k - segbytes, offset})\\
%18: \hspace{0.5cm}\emph{offset= offset + k - segBytes}\\
%19: \hspace{0.5cm}\textbf{K}=pack\_keys(\textbf{M, Seg},\emph{segBytes})\\
%20: \textbf{until} no segments left \\
%21: Output: Shuffled Index Array \textbf{O}\\
%\noindent\rule{16.5cm}{1.0pt}
